...
////*                    This Code is global data!                               *////     
        ...
        //This creates an array of all the enemy positions and then identifies the closest enemy
        var enemyPos = [];
        for(i = 0; i < enemyBots.length; i++) {
            enemyPos.push([enemyBots[i].pos.x, enemyBots[i].pos.y])
        };
        var closestEnemy = enemyPos[0]
        for(i = 0; i < enemyPos.length; i++) {
            if(bot.findDistance(myPos, closestEnemy) > bot.findDistance(myPos, enemyPos[i])) {
                closestEnemy = enemyPos[i];
            }
        };
        var task;
